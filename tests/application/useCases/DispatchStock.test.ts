...
describe("DispatchStock Use Case", () => {
  ...
  it("should not publish if skipPublishing is true", async () => {
    const sku = SKU.create("SKU-123");
    const item = InventoryItem.create("item-1", sku, Quantity.create(10));
    mockRepo.findBySku.mockResolvedValue(item);

    await useCase.execute("SKU-123", 5, true);
  ...
  it("should throw an error if item is not found in inventory", async () => {
    mockRepo.findBySku.mockResolvedValue(null);

    await expect(useCase.execute("MISSING-SKU", 5)).rejects.toThrow("Item not found in inventory");

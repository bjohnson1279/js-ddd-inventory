...
## 2025-02-14 - Fix SQL injection in TenantRegistry and RLS DDL
**Vulnerability:** Prisma $executeRawUnsafe and $queryRawUnsafe were used with string interpolation in TenantRegistry and TenantProvisioner for user input like tenantId, which opens up the potential for SQL Injection. For DDL operations in rls.ts, identifiers like table and column were not validated before being dynamically interpolated.
**Learning:** Prisma provides $executeRaw and $queryRaw combined with JS template literals to safely parameterize dynamic values. However, DDL operations in PostgreSQL (like ALTER TABLE and CREATE POLICY) cannot parameterize identifiers (table/column names).
**Prevention:** Always use safe parameterization ($executeRaw, $queryRaw) instead of unsafe raw versions with string interpolation. For DDL where parameterization is not possible, always validate dynamic identifiers against a strict regex (e.g., /^[a-zA-Z_][a-zA-Z0-9_]*$/) before interpolating them into the raw SQL string.

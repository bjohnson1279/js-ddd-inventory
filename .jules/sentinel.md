...
## 2024-07-12 - Fix Authorization Bypass in Organization Setup\n**Vulnerability:** The `/setup` endpoint in `AuthController.ts` allowed anyone to create a new admin user for an existing organization simply by providing its `tenantId` and a new admin email.\n**Learning:** The setup endpoint was designed to either create a new tenant or fetch the existing one if it was already created, but it failed to verify if the existing tenant had already been populated with users. This led to a critical authorization bypass and privilege escalation vulnerability where attackers could inject admin users into any known organization.\n**Prevention:** Always ensure that bootstrapping or setup endpoints enforce strict idempotency and authorization limits. If a setup endpoint fetches an existing entity, it must verify the entity's state (e.g., checking if `userModel.count > 0`) before allowing new high-privileged relationships to be created.

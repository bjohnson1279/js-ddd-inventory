...
## 2024-07-26 - Hardcoded Database Credential Fallbacks
**Vulnerability:** Hardcoded database connection strings defaulting to 'password' if the environment variable was missing (e.g. `process.env.DB_PASSWORD || 'password'`).
**Learning:** Default fallbacks in configuration for critical infrastructure elements like the database can silently expose sensitive components if secrets aren't properly passed in production or testing environments.
**Prevention:** Remove hardcoded fallbacks and omit the authentication segment entirely (e.g. `postgresql://user@host`) if a secret is missing. This ensures the connection fails securely and naturally with a generic error rather than silently attempting to authenticate with a common default credential.
## 2024-07-26 - Hardcoded Database Credential Fallbacks\n**Vulnerability:** Hardcoded database connection strings defaulting to 'password' if the environment variable was missing (e.g. `process.env.DB_PASSWORD || 'password'`).\n**Learning:** Default fallbacks in configuration for critical infrastructure elements like the database can silently expose sensitive components if secrets aren't properly passed in production or testing environments.\n**Prevention:** Remove hardcoded fallbacks and omit the authentication segment entirely (e.g. `postgresql://user@host`) if a secret is missing. This ensures the connection fails securely and naturally with a generic error rather than silently attempting to authenticate with a common default credential.

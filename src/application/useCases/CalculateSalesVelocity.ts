...
export class CalculateSalesVelocity {
  ...
  async execute(skuStr: string, locationId: string = "default", preFetchedStock?: number, preFetchedHistory?: DispatchRecord[]): Promise<SalesVelocityResult> {
    const now = new Date();
    ...
    const history90d = preFetchedHistory || await this.dispatchRecordRepository.fetchHistory(skuStr, locationId, ninetyDaysAgo);

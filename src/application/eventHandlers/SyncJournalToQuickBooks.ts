  ...
  try {
    ...
    if (existing) {
      console.info(JSON.stringify({
        context: "QuickBooks Sync",
        message: `Local journal ${event.aggregateId} already synced to QuickBooks.`,
    ...
    console.info(JSON.stringify({
      context: "QuickBooks Sync",
      message: `Successfully mapped local journal ${event.aggregateId} -> QuickBooks ${qbId}`,
      journalEntryId: event.aggregateId,
      qbId
    }));
  } catch (err: any) {

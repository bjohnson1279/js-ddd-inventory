...
const start = async () => {
  ...
  try {
    ...
    Logger.info({ context: "Bootstrap", message: "TimescaleDB extension enabled."  });
    ...
    if ((isHypertable as any[]).length === 0) {
      ...
      Logger.info({ context: "Bootstrap", message: "dispatch_records table converted to TimescaleDB hypertable."  });
    }
    ...
    if ((isView as any[]).length === 0) {
      ...
      try {
        ...
        Logger.warn({ context: "Bootstrap", message: "TimescaleDB aggregate policy setup warning", error: policyErr.message  });
      }
      Logger.info({ context: "Bootstrap", message: "daily_dispatch_summary continuous aggregate created."  });
    }
    
    // Set up PostgreSQL Row-Level Security (RLS) policies
    await enableRowLevelSecurity(prisma);
  } catch (e) {
    Logger.warn({ context: "Bootstrap", message: "Database/TimescaleDB setup skipped/warning", error: (e as Error).message  });
  }

  if (process.env.DB_HOST) {
    Logger.info({ context: "Bootstrap", message: "Initializing PostgreSQL Repository..."  });
    ...
    Logger.info({ context: "Bootstrap", message: "Initializing Prisma Repository..."  });
  ...
  const server = app.listen(port, () => {
    Logger.info({ context: "Bootstrap", message: `Server is running on port ${port}` });
  });
...
if (process.env.NODE_ENV !== "test") {
  start().catch((err) => {
    Logger.error({ context: "Bootstrap", message: "Failed to start server", error: err instanceof Error ? err.message : String(err)  });
    process.exit(1);
  });
}

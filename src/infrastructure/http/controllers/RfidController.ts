...
export class RfidController {
  static async list(req: Request, res: Response) {
    try {
      ...
      Logger.error({ context: "RfidController", message: "An error occurred", error: error.message });
    ...
    try {
      ...
      client.on("connect", () => {
        client.publish(`tenants/${tenantId}/rfid/scans`, JSON.stringify(payload), { qos: 0 }, (err) => {
          client.end();
          if (err) {
            Logger.error({ context: "RfidController", message: "Failed to publish MQTT message", error: err.message });
      ...
      client.on("error", (err) => {
        client.end();
        Logger.error({ context: "RfidController", message: "MQTT Connection Error", error: err.message });

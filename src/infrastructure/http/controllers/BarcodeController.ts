...
export class BarcodeController {
  static async assign(req: Request, res: Response) {
    try {
      const barcodeRepo = req.app.get("barcodeRepository") as IBarcodeRepository;
      const { variantId, symbology, barcodeValue, source, isPrimary } = req.body;
      ...
      res.status(200).json({ message: "Barcode assigned successfully.", variantId, barcodeValue });
    } catch (error: any) {
  ...
  static async generate(req: Request, res: Response) {
    try {
      ...
      const barcode = await generator.generate(variantId, tenantId || "DEFAULT");
  ...
  static async scan(req: Request, res: Response) {
    try {
      ...
      if (!rawScan || !context) {
        return res.status(400).json({ error: "Missing rawScan or scan context." });
      }
